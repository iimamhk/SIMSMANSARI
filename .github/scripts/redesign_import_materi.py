...
css_pattern = re.compile(r"    \.mi-info-card \{.*?    \.mi-info-card li \{ margin:2px 0; \}", re.S)
css_new = """    .mi-guide { margin-top:12px; padding:14px; border:1px solid #e2e8f0; border-radius:18px; background:linear-gradient(135deg,#ffffff,#f8fafc); box-shadow:0 12px 30px -26px rgba(15,23,42,.35); }
...
text, css_count = css_pattern.subn(css_new, text, count=1)
if css_count != 1:
    raise SystemExit(f'CSS block replacement failed: {css_count}')

start = text.find('    <div class=\"mi\">\n    <section class=\"mi-hero\">')
end_marker = '    <div class=\"mi-meta\">'
end = text.find(end_marker, start)
if start < 0 or end < 0:
    raise SystemExit('Import intro block not found')

new_intro = '''    <div class="mi">
...
if leftovers:
    raise SystemExit('Old copy still present')

path.write_text(text, encoding='utf-8')
print('materi-import.js updated')
