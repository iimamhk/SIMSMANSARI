...
async function createSpreadsheet(sheets, drive, parentFolderId, guruId, guruName, dateStr) {
  ...
  if (process.env.BACKUP_SHARE_EMAIL) {
    try {
      ...
      console.log(`  Shared with ${process.env.BACKUP_SHARE_EMAIL}`);
    } catch (e) {
...
async function generateRekapAbsensi(sheets, spreadsheetId, assignment, siswaList, absensiData, sheetIndex) {
  ...
  const totalRowIndex = values.length - 1;
  await sheets.spreadsheets.batchUpdate({
    spreadsheetId,
    resource: {
      requests: [
        {
          repeatCell: {
            range: { sheetId, startRowIndex: totalRowIndex, endRowIndex: totalRowIndex + 1, startColumnIndex: 0, endColumnIndex: values[0].length },
        ...
        {
          repeatCell: {
            range: { sheetId, startRowIndex: 1, endRowIndex: totalRowIndex, startColumnIndex: 2, endColumnIndex: values[0].length - 2 },
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    resource: {
      requests: [
        ...
        {
          repeatCell: {
            range: { sheetId, startRowIndex: 1, endRowIndex: totalRowIndex, startColumnIndex: 2, endColumnIndex: 2 + dateCount },
            cell: { userEnteredFormat: { horizontalAlignment: 'CENTER' } },
            fields: 'userEnteredFormat(horizontalAlignment)',
          },
        },
        {
          repeatCell: {
            range: { sheetId, startRowIndex: 1, endRowIndex: totalRowIndex, startColumnIndex: 2 + dateCount, endColumnIndex: values[0].length },
            cell: { userEnteredFormat: { horizontalAlignment: 'CENTER' } },
            fields: 'userEnteredFormat(horizontalAlignment)',
          },
        },
...
async function generateNilai(sheets, spreadsheetId, assignment, siswaList, nilaiData, babData, tugasData, uhKolomData, sheetIndex) {
  ...
  const headerRows = mergedHeaders.length + secondLevelHeaders.length;
  const headerStartRow = configValues.length + 1;
  const dataStartRow = headerStartRow + mergedHeaders.length + 1;
  ...
  requests.push({
    repeatCell: {
      ...
      cell: {
        userEnteredFormat: {
          backgroundColor: { red: 0.12, green: 0.28, blue: 0.45 },
  ...
  requests.push({
    autoResizeDimensions: {
      dimensions: { sheetId, dimension: 'COLUMNS', startIndex: 0, endIndex: currentColumn - 1 },
    },
  });
...
async function main() {
  try {
    ...
    if (process.env.BACKUP_SHARE_EMAIL && rootFolder) {
      try {
        console.log(`Sharing root backup folder with ${process.env.BACKUP_SHARE_EMAIL}...`);
        await drive.permissions.create({
          ...DRIVE_API_OPTIONS,
        ...
        console.log(`  Root backup folder shared.`);
      } catch (e) {
        console.warn(`  Could not share root folder: ${e.message}`);
      }
    }
    ...
    if (process.env.BACKUP_SHARE_EMAIL) {
      try {
        console.log(`Sharing date folder with ${process.env.BACKUP_SHARE_EMAIL}...`);
        await drive.permissions.create({
          ...DRIVE_API_OPTIONS,
        ...
        console.log(`  Date folder shared.`);
      } catch (e) {
        console.warn(`  Could not share date folder: ${e.message}`);
      }
    }
    ...
    for (const guruId of Object.keys(assignmentsByGuru)) {
      ...
      siswaList = [...new Map(siswaList.map(item => [item.id, item])).values()];
