...
function pageStyles() {
  return `
    ...
    .mw-workspace-nav { position:relative; display:flex; gap:4px; margin-top:14px; padding:5px; border:1px solid rgba(226,232,240,.9); border-radius:20px; background:linear-gradient(135deg,rgba(248,250,252,.97),rgba(241,245,249,.85)); box-shadow:0 14px 34px -24px rgba(15,23,42,.35), inset 0 1px 0 rgba(255,255,255,.85); overflow-x:auto; scrollbar-width:none; }
    .mw-workspace-nav::-webkit-scrollbar { display:none; }
    .mw-nav-btn { position:relative; flex:1 1 0; min-width:max-content; display:inline-flex; flex-direction:column; align-items:center; gap:3px; border:1px solid transparent; border-radius:15px; padding:10px 14px 9px; background:transparent; color:#64748b; font-size:11.5px; font-weight:700; letter-spacing:.005em; cursor:pointer; transition:all .28s cubic-bezier(.22,1,.36,1); white-space:nowrap; }
    .mw-nav-ico { display:inline-flex; align-items:center; justify-content:center; width:34px; height:34px; border-radius:11px; background:rgba(100,116,139,.08); color:#64748b; font-size:16px; line-height:1; transition:all .28s cubic-bezier(.22,1,.36,1); }
    .mw-nav-label { display:block; line-height:1.2; transition:color .28s; }
    .mw-nav-badge { position:absolute; top:6px; right:8px; min-width:16px; height:16px; padding:0 4px; border-radius:999px; background:#ef4444; color:#fff; font-size:9px; font-weight:800; display:inline-flex; align-items:center; justify-content:center; line-height:1; box-shadow:0 2px 6px rgba(239,68,68,.4); }
    .mw-nav-btn:hover { color:#334155; background:rgba(255,255,255,.7); transform:translateY(-1px); }
    .mw-nav-btn:hover .mw-nav-ico { background:rgba(37,99,235,.12); color:var(--mw-primary); transform:scale(1.06); }
    .mw-nav-btn:focus-visible { outline:3px solid rgba(37,99,235,.25); outline-offset:2px; }
    .mw-nav-btn.active { background:linear-gradient(135deg,var(--mw-primary),#4f46e5); color:#fff; border-color:transparent; box-shadow:0 14px 30px -10px rgba(37,99,235,.5), inset 0 1px 0 rgba(255,255,255,.3); transform:translateY(-2px); }
    .mw-nav-btn.active .mw-nav-ico { background:rgba(255,255,255,.22); color:#fff; transform:scale(1.1); box-shadow:0 4px 10px rgba(0,0,0,.12); }
    .mw-nav-btn.active .mw-nav-label { font-weight:800; }
    .mw-nav-btn.active .mw-nav-badge { background:#fff; color:var(--mw-primary); box-shadow:0 2px 6px rgba(0,0,0,.18); }
    @media (max-width:640px) { .mw-nav-btn { flex:none; padding:9px 12px 8px; } .mw-nav-ico { width:30px; height:30px; font-size:14px; } .mw-nav-label { font-size:10.5px; } }
...
export async function renderGuruMateriPage(container) {
  ...
  const html = renderLayout('Materi', `<style>${pageStyles()}</style><div class="mw" style="--mw-primary:${doc.theme.primary};--mw-bg:${doc.theme.background};--mw-surface:${doc.theme.surface};--mw-text:${doc.theme.text}">
    <nav class="mw-workspace-nav" aria-label="Workspace materi"><button class="mw-nav-btn active" data-mode="home"><span class="mw-nav-ico">⌂</span><span class="mw-nav-label">Beranda</span></button><button class="mw-nav-btn" data-mode="drafts"><span class="mw-nav-ico">✎</span><span class="mw-nav-label">Draft</span><span class="mw-nav-badge" id="mw-nav-draft-count" hidden>0</span></button><button class="mw-nav-btn" data-mode="published"><span class="mw-nav-ico">▤</span><span class="mw-nav-label">Terbit</span></button><button class="mw-nav-btn" data-mode="editor"><span class="mw-nav-ico">＋</span><span class="mw-nav-label">Buat Materi</span></button><button class="mw-nav-btn" data-mode="ai" data-nav-href="#guru/materi-ai"><span class="mw-nav-ico">✦</span><span class="mw-nav-label">Materi AI</span></button><button class="mw-nav-btn" data-mode="import" data-nav-href="#guru/materi-import"><span class="mw-nav-ico">📥</span><span class="mw-nav-label">Import</span></button></nav>
  ...
  const cloneDraft = async (id) => { const draft = remoteDrafts.find((item) => String(item.id) === String(id)); if (!draft?.document_json) return; doc = normalizeDocument(JSON.parse(JSON.stringify(draft.document_json))); doc.id = uid('materi'); doc.meta.title = `${doc.meta.title || 'Materi'} (Salinan)`; selectedId = doc.blocks[0]?.id || ''; history = [JSON.stringify(doc)]; historyIndex = 0; await persistDraft(true); remoteDrafts.unshift({ id: doc.id, title: doc.meta.title, subject: doc.meta.subject, class_name: doc.meta.className, updated_at: doc.updated_at || new Date().toISOString(), document_json: doc }); updateDraftBadge(); setMode('editor'); render(); };
