...
export async function renderGuruMateriPage(container) {
  ...
  const renderLibrary = () => {
    ...
    const body = entries.length
      ...
      : `<div class="mw-ios-empty"><span class="mw-ios-empty-icon">${listQuery ? '⌕' : '▤'}</span><h3>${listQuery ? 'Materi tidak ditemukan' : 'Belum ada materi'}</h3><p>${listQuery ? 'Coba kata kunci judul, mapel, atau kelas yang lain.' : 'Mulai dari Materi AI atau Import Materi.'}</p></div>`;
