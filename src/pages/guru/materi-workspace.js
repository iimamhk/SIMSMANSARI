...
export async function renderGuruMateriPage(container) {
  ...
  const renderOverview = () => { if (mode === 'drafts') { renderDrafts(); return; } const copy = mode === 'home' ? ['Kelola materi pembelajaran dalam satu workspace.', 'Buat materi baru, lanjutkan draft, lalu siapkan distribusi ke kelas.'] : ['Terbit & Distribusi', 'Manajemen publish multi-kelas akan aktif setelah penyimpanan JSON fase berikutnya.']; overview.innerHTML = `<div class="mw-overview-head"><div><p class="mw-eyebrow">Material Workspace</p><h1>${copy[0]}</h1><p>${copy[1]}</p></div><button class="mw-action-btn primary" data-open-editor>+ Buat Materi</button></div>`; overview.querySelector('[data-open-editor]')?.addEventListener('click', () => setMode('editor')); };
