...
function pageStyles() {
  return `
    ...
    .mw-workspace-nav { display:flex; gap:6px; margin-top:12px; padding:6px; border:1px solid rgba(226,232,240,.9); border-radius:18px; background:linear-gradient(135deg,rgba(248,250,252,.97),rgba(239,246,255,.8)); box-shadow:0 12px 30px -22px rgba(15,23,42,.35), inset 0 1px 0 rgba(255,255,255,.8); overflow-x:auto; scrollbar-width:none; }
    .mw-workspace-nav::-webkit-scrollbar { display:none; }
    .mw-nav-btn { position:relative; flex:1 1 0; min-width:max-content; display:inline-flex; align-items:center; justify-content:center; gap:8px; border:1px solid transparent; border-radius:13px; padding:11px 16px; background:transparent; color:#64748b; font-size:12.5px; font-weight:750; letter-spacing:.01em; cursor:pointer; transition:all .25s cubic-bezier(.22,1,.36,1); white-space:nowrap; }
    .mw-nav-btn .mw-nav-ico { display:inline-flex; align-items:center; justify-content:center; width:26px; height:26px; border-radius:8px; background:rgba(100,116,139,.1); font-size:13px; line-height:1; transition:all .25s; }
    .mw-nav-btn:hover { color:#334155; background:rgba(255,255,255,.85); border-color:rgba(226,232,240,.9); transform:translateY(-1px); }
    .mw-nav-btn:focus-visible { outline:3px solid rgba(37,99,235,.25); outline-offset:2px; }
    .mw-nav-btn.active { background:linear-gradient(135deg,var(--mw-primary),#4f46e5); color:#fff; border-color:transparent; box-shadow:0 12px 26px -10px rgba(37,99,235,.55), inset 0 1px 0 rgba(255,255,255,.25); transform:translateY(-1px); }
    .mw-nav-btn.active .mw-nav-ico { background:rgba(255,255,255,.2); }
    .mw-nav-btn::after { content:''; position:absolute; inset:auto 14px 4px; height:3px; border-radius:999px; background:currentColor; opacity:0; transform:scaleX(.3); transition:opacity .25s, transform .25s; }
    .mw-nav-btn.active::after { opacity:.85; transform:scaleX(1); }
    @media (max-width:640px) { .mw-nav-btn { flex:none; padding:10px 14px; } }
    ...
    .mw-ios-hero { position:relative; overflow:hidden; display:flex; align-items:center; justify-content:space-between; gap:14px; min-height:96px; padding:18px 22px; border-radius:18px; color:#fff; background:linear-gradient(125deg,#1c1c1e 0%,#2c2c2e 42%,#0a84ff 120%); }
    .mw-ios-hero-copy { position:relative; z-index:1; }
    .mw-ios-kicker { display:flex; align-items:center; gap:6px; margin:0 0 6px; color:rgba(255,255,255,.68); font-size:10px; font-weight:800; letter-spacing:.14em; text-transform:uppercase; }
    .mw-ios-kicker span { width:24px; height:24px; display:inline-flex; align-items:center; justify-content:center; border:1px solid rgba(255,255,255,.2); border-radius:8px; background:rgba(255,255,255,.1); font-size:12px; }
    .mw-ios-hero h1 { max-width:520px; margin:0; font-size:clamp(1.15rem,2.5vw,1.6rem); line-height:1.1; letter-spacing:-.035em; }
    .mw-ios-hero-stats { display:flex; flex-wrap:wrap; gap:6px; margin-top:10px; }
    .mw-ios-stat { display:inline-flex; align-items:center; gap:6px; padding:5px 9px; border:1px solid rgba(255,255,255,.14); border-radius:999px; background:rgba(255,255,255,.1); color:rgba(255,255,255,.88); font-size:10px; font-weight:700; }
...
export async function renderGuruMateriPage(container) {
  ...
  const renderHomeLibrary = (query = '') => {
    ...
    overview.innerHTML = `<div class="mw-ios-home"><section class="mw-ios-hero"><div class="mw-ios-hero-copy"><p class="mw-ios-kicker"><span>▤</span> Perpustakaan Materi</p><h1>Halo, ${firstName}.</h1><div class="mw-ios-hero-stats"><span class="mw-ios-stat">${groups.length} judul</span><span class="mw-ios-stat">${publishedMaterials.length} distribusi</span><span class="mw-ios-stat">${remoteDrafts.length} draft</span></div></div></section><section class="mw-ios-library"><div class="mw-ios-library-head"><div><p class="mw-eyebrow">Koleksi Saya</p><h2>${mode === 'published' ? 'Terbit & Distribusi' : 'Semua Buku'}</h2><p>${visibleGroups.length} dari ${groups.length} judul.</p></div><div class="mw-ios-tools"><label class="mw-ios-search"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"></circle><path d="m21 21-4.35-4.35"></path></svg><input id="mw-book-search" type="search" value="${escapeAttr(query)}" placeholder="Cari materi" aria-label="Cari materi"></label><button class="mw-action-btn primary" data-open-editor>+ Buku Baru</button></div></div>${books}</section></div>`;
    overview.querySelector('[data-open-editor]')?.addEventListener('click', () => setMode('editor'));
